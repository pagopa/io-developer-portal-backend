...
export const IREQUEST_REVIEW_LEGACY_QUEUE_CONFIG = t.interface({
  REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_CONNECTIONSTRING: NonEmptyString,
  REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_NAME: NonEmptyString
});
...
export const getRequestReviewLegacyQueueConfigOrThrow = () =>
  IREQUEST_REVIEW_LEGACY_QUEUE_CONFIG.decode({
    ...process.env,
    REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_CONNECTIONSTRING:
      process.env.REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_CONNECTIONSTRING,
    REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_NAME:
      process.env.REQUEST_REVIEW_LEGACY_STORAGE_QUEUE_NAME
  }).getOrElseL(err => {
    throw new Error(errorsToReadableMessages(err).join("|"));
  });
